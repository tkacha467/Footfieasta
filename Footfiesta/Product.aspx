...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="server">
    <asp:DataList ID="DataList1" runat="server" BorderColor="#003300"
        ...
        <ItemTemplate>
            <div style="border: 1px solid #003300; padding: 15px; border-radius: 10px; background-color: #f7fdfc; box-shadow: 2px 2px 10px rgba(0, 0, 0, 0.1); display: inline-block; width: 220px; height: 380px; margin: 10px; text-align: center; vertical-align: top; overflow: hidden;">
                ...
                <br />
                <div id='<%# "desc_" + Eval("Product_Id") %>'
                    style="font-size: 14px; color: #666; max-height: 40px; overflow: hidden;">
                    <asp:Literal ID="litDescription" runat="server" Text='<%# FormatDescription(Eval("Description").ToString(), Eval("Product_Id").ToString()) %>'></asp:Literal>
                </div>
                ...
                <asp:LinkButton ID="LinkButton1" runat="server" CommandArgument='<%# Eval("Product_Id") %>'
                    ...
                    Style="display: block; padding: 8px 15px; background-color: #008080; color: white; text-decoration: none; border-radius: 5px; font-size: 14px; margin-bottom: 5px;">View Details</asp:LinkButton>
                <asp:LinkButton ID="LinkButton2" runat="server" CommandArgument='<%# Eval("Product_Id") %>'
                    CommandName="cmd_adtc"
                    Style="display: block; padding: 8px 15px; background-color: #004d4d; color: white; text-decoration: none; border-radius: 5px; font-size: 14px;">Add To Cart</asp:LinkButton>
            </div>
        </ItemTemplate>
    </asp:DataList>
    ...
    <script>
        function showFullDescription(id) {
            ...
            element.innerHTML = element.getAttribute("data-full");
        }
    </script>
</asp:Content>
